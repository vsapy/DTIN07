...
#The G2 neurons receive the input spike from the G1 neurons and have a recurrent connection that feeds the spike back
# with a delay of 2*bits_per_slot*ms so that it arrives in pahse with the next spike from the G1 neurons.  Using the refractory
# property of the neuron the first spike to arrive is proagated and the second spike is suppressed.  The result at the
# end of Num_bound cycles the earliest spike in any of the cycles is preserved.

G2 = NeuronGroup(slots_per_vector, equ2, threshold= 'v>=0.5', reset='v=0.0', method='euler',refractory ='t%(2*bits_per_slot*ms)')
...
S22.connect(j='i')
S22.delay = (2*bits_per_slot-0.1)*ms
...
# time delay in each slot plus the delay matrix line up at the number of bits_per_slot per slot
...
# NeuronGroup G7 is a recurrent circuit which simply repeates the sparse bound vector from P1 every 3*bits_per_slot milliseconds
...
G8 = NeuronGroup(mem_size, equ2, threshold='v >= 13.0', reset='v=0.0', method='euler')
